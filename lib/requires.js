    ...
    return (function ($$events, $$crypto, $$path, $$util, $$url, $$buffer, fetch) {
      ...
      function makeContext(contextName) {
        ...
        mainRequire.home = function (basePath) {
          ...
          if (!basePath && had) return context.basePath; else if(!basePath) basePath = "./";
          ...
          if(had && !(basePath.indexOf("../") == 0 || basePath.indexOf("./") == 0))
            context.basePath = context.parsePath(basePath).href;
          else{
            if(!had)
              context.basePath = context.parsePath(__callerFile, basePath).href;
            else 
              context.basePath = context.parsePath(context.basePath, basePath).href;
          }
          if (had)
            mainRequire.home = u;
        };
        ...
        context.provider = preDefinedProviders[preDefinedProviders.default].basePath;
        mainRequire.provider = function (packageProvider, providerConfig) {
          ...
          // var __callerFile = _getCallerFile();
          var packageProviderURL = context.parsePath(context.basePath, providerConfig && providerConfig.basePath || packagePath);

          if (packagePath == "./node_modules/") {
            providerConfig.basePath = packageProviderURL.href;
          }
