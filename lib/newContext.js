...
module.exports = function _Context($$fetch, $$exec, $$global) {
  ...
  var commentRegExp = /\/\*[\s\S]*?\*\/|([^:"'=]|^)\/\/.*$/gm,
    ...
    functionArgsNames = /(?<=\()(.*?)(?=\))/; // jsSuffixRegExp = /\.js$/,
  // jsExtRegExp = /^\/|:|\?|\.js$/;
  ...
  Dependency.prototype.setPackagePath = function setPackagePath(customPath) {
    ...
    if (customPath) {
      ...
      if (this.context.provider_config) packagePath += "@" + this.version;
    }
  ...
  Dependency.prototype.loadPackage = function loadPackage(parrentPackage) {
    ...
    return new Promise(async function dependencyLoad(resolve) {
      ...
      $$fetch(
        ...
        async function dependencyLoad(packageJSON, resolvedURL, status) {
          ...
          if (status !== "200") {
            if (!path.basename(context.provider) == "node_modules") {
              ...
              throw ""
                .concat(dependency.package, " status is ")
                .concat(status, " and needs to be 200");
            }
          ...
          if (packageJSON.version !== dependency.version) {
            if (semver.satisfies(packageJSON.version, dependency.version)) {
              dependency.version = packageJSON.version; // console.warn(dependency.name, "version changed", dependency._version, dependency.version);
            } else {
              if (!path.basename(context.provider) == "node_modules") {
                //this should never happen.. as we requested this version.
                throw ""
                  .concat(packageJSON.name, "@")
                  .concat(packageJSON.version, " doesnt satisfy ")
  ...
  Dependency.prototype.addDependency = function addDependency(
    ...
    var promise = new Promise(async (resolve) => {
      ...
      if (Dependency._cached[dependency.name + "@" + dependency.version]) {
        dependency =
          Dependency._cached[dependency.name + "@" + dependency.version]; // console.warn("Re-Used Dependency", dependencyName + "@" + dependencyVersion, "for", parentDependency.name +"@"+parentDependency.version);
      } else {
        ...
        if (
          ...
          dependency.attachTo(contextDependency); // console.warn("Added Dependency", dependencyName + "@" + dependencyVersion, "for", parentDependency.name +"@"+parentDependency.version);
      }
  ...
  Module.prototype.load = async function /*callerModule*/
  load() {
    var module = this;
    ...
    var promise = new Promise(async (resolve) => {
      ...
      if (module.transformed) {
        ...
        try {
          ...
          var $$exported;
          var $$exported_async = module.transformed(
            defineRequire,
            $module.exports,
            $module,
            function reDefine($$name, $$deps, $$definition) {
              reDefined = true;
          ...
          if (!reDefined && $$exported instanceof Promise)
            $$exported = (await $$exported_async).exports;
        } catch (e) {
          var errorLineNumber;
  ...
  Context.prototype.install = function install(packageURL, callback) {
    ...
    var promise = new Promise(function contextInstall(resolve) {
      ...
      if (packageURL.substr(0 - "package.json".length) == "package.json") {
        packageURL = packageURL.substr(
          0,
          packageURL.length - "package.json".length
        );
      }
      ...
      $$fetch(
        packageURL_JSON,
        async function contextInstall(packageJSON, resolvedURL) {
          if (packageURL_JSON != resolvedURL)
            console.warn("INSTALL:", packageURL_JSON, "!=", resolvedURL);
          packageJSON = JSON.parse(packageJSON);
          packageDependency = new Dependency(
            context,
            packageJSON.name,
            packageJSON.version
          );
  ...
  Context.prototype.defineModule = async function defineModule(
    moduleName,
    deps,
    transformed
  ) {
  ...
  var parsePath = (Context.prototype.parsePath = function parsePath(
    baseFile,
    $path
  ) {
    // var context = this;
  ...
  function _getCallerFile() {
    ...
    try {
      ...
      while (err.stack.length) {
        ...
        if ($callerfile.indexOf("node:") == 0) continue;
