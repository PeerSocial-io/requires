...
module.exports = function _Context($$fetch, $$exec) {
  ...
  Dependency.prototype.addDependency = function addDependency(dependencyName, dependencyVersion) {
    ...
    var promise = new Promise(async (resolve) => {
      ...
      if (parentDependency.dependencies[dependencyName])//this is rare but just in cases we load it again for the same dep
        return resolve(parentDependency.dependencies[dependencyName]);

      if(Dependency._cached[dependencyName + "@" + dependencyVersion]){//re used from cache, add to parent dep
        if(!parentDependency.dependencies[dependencyName] || !parentDependency.dependencies[dependencyName][dependencyVersion]){
          if(!parentDependency.dependencies[dependencyName])
            parentDependency.dependencies[dependencyName] = {};
          parentDependency.dependencies[dependencyName][dependencyVersion] = Dependency._cached[dependencyName + "@" + dependencyVersion];
        }
        console.warn("reused Dependency", dependencyName + "@" + dependencyVersion, "for", parentDependency.name +"@"+parentDependency.version);
        return resolve(Dependency._cached[dependencyName + "@" + dependencyVersion]);
      }

      var dependency = new Dependency(context, dependencyName, dependencyVersion, parentDependency);
      // contextDependency;

      Dependency._cached[dependencyName + "@" + dependencyVersion] = dependency;
  ...
  Context.prototype.install = async function install(packageURL, callback) {
    ...
    var promise = new Promise(function contextInstall(resolve) {
      ...
      $$fetch(packageURL_JSON, async function contextInstall(packageJSON, resolvedURL) {
        if(packageURL != resolvedURL)
          console.warn("INSTALL:", packageURL, "!=", resolvedURL);
        packageJSON = JSON.parse(packageJSON);
