...
module.exports = function _Context($$fetch, $$exec, $$global) {
  ...
  Dependency.prototype.loadPackage = function loadPackage(parrentPackage) {
    var dependency = this;
    this.registry =
      (parrentPackage && parrentPackage.registry) || this.registry;
    ...
    return new Promise(async function dependencyLoad(resolve) {
      ...
      if (!registry_package) throw new Error("Package Failed to load");
  ...
  Context.prototype.install = function install(packageURL, callback) {
    ...
    var promise = new Promise(async function contextInstall(resolve) {
      ...
      if (["file", "http", "https"].indexOf(packageURL_ModPath.registry) >=0) {
        ...
        packageDependency.setRegistry("npmjs");

        if (
          packageURL_ModPath.name == packageURL ||
          packageURL_ModPath.name + "@" + packageURL_ModPath.version ==
            packageURL
        ) {
  ...
  Context.prototype.parsePath = function parsePath($$baseFile, $$pathOverRide) {
    ...
    if (
      baseFile.protocol &&
      ["file:", "http:", "https:"].indexOf(baseFile.protocol) == -1
    )
